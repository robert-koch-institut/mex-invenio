...
def merge_translations(
    ...
    try:
        ...
        return
    print(
        f"Processing language: {language}\nLoaded {len(base_po)} entries from mex-model {language}.po (base)"
    )

    if merge_ui:
        # Load ui.po (override translations)
        ui_po_path = os.path.join(
            instance_path, f"translations/{language}/LC_MESSAGES/ui.po"
        )
    ...
    if base_po.metadata:
        ...
        messages_po.metadata["POT-Creation-Date"] = current_time
        messages_po.metadata["PO-Revision-Date"] = current_time
    ...
    for base_entry in base_po:
        ...
        if merge_ui and key in ui_lookup:
            ...
            new_entry = polib.POEntry(
                ...
                + base_entry.occurrences,  # Combine occurrences
    ...
    # Save merged file
    messages_po_path = os.path.join(
        instance_path, f"translations/{language}/LC_MESSAGES/messages.po"
    )
    ...
    if merge_ui and ui_only > 0:
        ...
        for entry in messages_po:
            if entry.msgid:
                key = create_entry_key(entry)
                if (
                    key not in base_keys_processed and only_count < 5
