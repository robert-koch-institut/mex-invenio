...
def bulk_search_existing_records(mex_ids: list[str], identity) -> dict[str, dict]:
    """Search for multiple MEx IDs using direct database query."""
    if not mex_ids:
        return {}

    try:
        # Query database with JOIN to get PID in one query
        ...
        for record, pid_value in records_with_pids:
            mex_id = record.json.get('custom_fields', {}).get('mex:identifier')
            if mex_id:
                ...
                record_data = {
                    'id': pid_value,
                    'custom_fields': record.json.get('custom_fields', {}),
                    'metadata': record.json.get('metadata', {})
                }
                existing_records[mex_id] = record_data

        logger.debug(f"Found {len(existing_records)} existing records out of {len(mex_ids)} requested")
...
@click.option(
    "--re-index", is_flag=True, default=False, help="Rebuild search index after import."
)
def _import_data(email: str, filepath: str, batch_size: int, re_index: bool):
    return import_data(email, filepath, batch_size, re_index, cli=True)
...
def import_data(
    ...
    num_lines = 0
    report = {"created": [], "updated": [], "skipped": [], "error": 0}
