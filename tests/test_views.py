...
def test_mex_record_view_as_json(
    ...
    assert response.status_code == 200, f"JSON endpoint should return 200, got {response.status_code}"
    
    # Verify content type is JSON
    assert response.content_type == "application/json", f"Expected JSON content type, got {response.content_type}"
    ...
    assert "custom_fields" in json_result, "JSON result should contain 'custom_fields' field"
