    ...
    def test_entity_required_keys(self, app, app_config):
        ...
        for entity_name, entity_config in ui_settings.items():
            for key in required_keys:
                assert key in entity_config, (
                    f"Missing {key} in {entity_name} configuration"
                )
        ...
        for entity_name, entity_config in ui_settings.items():
            ...
            for section in sections:
                for card_id, card_config in entity_config[section].items():
                    ...
                    assert "title" in card_config, (
                        f"Missing title in {entity_name}.{section}.{card_id}"
                    )
                    assert "icon" in card_config, (
                        f"Missing icon in {entity_name}.{section}.{card_id}"
                    )
        ...
        for entity_name, entity_config in ui_settings.items():
            ...
            for field_key, field_config in special_fields.items():
                assert "field" in field_config, (
                    f"Missing field in {entity_name}.special_fields.{field_key}"
                )
                assert isinstance(field_config["field"], str), (
                    f"Field must be string in {entity_name}.special_fields.{field_key}"
                )
                ...
                for card_id, card_config in entity_config[section].items():
                    if "properties" in card_config:
                        ...
                        assert isinstance(properties, list), (
                            f"Properties must be list in {entity_name}.{section}.{card_id}"
                        )

                        for prop in properties:
                            assert "field" in prop, (
                                f"Missing field in property in {entity_name}.{section}.{card_id}"
                            )
                            assert isinstance(prop["field"], str), (
                                f"Field must be string in {entity_name}.{section}.{card_id}"
                            )
            ...
            for section in sections:
                for card_id, card_config in entity_config[section].items():
                    if card_config.get("type") == "container":
                        assert "components" in card_config, (
                            f"Container card missing components in {entity_name}.{section}.{card_id}"
                        )

                        components = card_config["components"]
                        assert isinstance(components, list), (
                            f"Components must be list in {entity_name}.{section}.{card_id}"
                        )
                        ...
                        for component in components:
                            ...
                            if "type" in component:
                                assert component["type"] == "component", (
                                    f"Component type must be 'component' in {entity_name}.{section}.{card_id}"
                                )
                            # Some components may not have 'title' field - that's ok too
                            assert "properties" in component, (
                                f"Component missing properties in {entity_name}.{section}.{card_id}"
                            )
    ...
    def test_field_types_structure(self, app, app_config):
        ...
        for entity_type, fields in field_types.items():
            assert isinstance(fields, dict), (
                f"Field types for {entity_type} must be dict"
            )

            for field_name, field_type in fields.items():
                assert isinstance(field_type, str), (
                    f"Field type for {entity_type}.{field_name} must be string"
                )
                ...
                assert field_type in valid_types, (
                    f"Invalid field type '{field_type}' for {entity_type}.{field_name}"
                )
...
class TestConfigurationIntegrity:
    ...
    def test_field_references_exist_in_field_types(self, app, app_config):
        ...
        for entity_name, entity_config in ui_settings.items():
            ...
            for field_key, field_config in entity_config["special_fields"].items():
                ...
                if (
                    entity_name in field_types
                    and field_name in field_types[entity_name]
                ):
                    # Field exists - good
                    pass
    ...
    def test_title_fields_exist_in_field_types(self, app, app_config):
        ...
        assert len(found_title_fields) > 0, (
            f"No title fields found in FIELD_TYPES. Expected at least one of: {title_fields}"
        )
